			...
			<div class="layui-form" style="padding: 2px 120px;margin-top: -10px;" >
				<div class="layui-row">
					<div class="layui-form-item">
						...
						<input type="text" name="teacher_id" title="老师id" style="display:none;"value="${courseList.teacher_id}">
							<input type="text" name="coursename" id="coursename" lay-verify="title"
									autocomplete="off" placeholder="请输入课件包名称" class="layui-input" value="${courseList.coursename}">
						</div>
					</div>
					<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="text" name="knowledge" id="knowledge" lay-verify="title"
									autocomplete="off" placeholder="请输入知识点" class="layui-input" value="${courseList.knowledge}">
							</div>
					</div>
					<div class="layui-form-item">
								...
								<div class="layui-input-block"style="width:320px;" >
									<select name="teachversionId" id="teachversionId">
									     ...
									     <c:forEach items="${teachversionList}" var="teachversionList">
											<option value="${teachversionList.teachversionId}" ${teachversionList.teachversionId eq courseList.teachversionId? "selected":""}>${teachversionList.teachversion}</option>
										 </c:forEach>
									</select>
								</div>
						</div>
								...
								<div class="layui-input-block" style="width:320px;">
									<select name="gradeId" id="gradeId">
									     ...
									     <c:forEach items="${gradeList}" var="gradeList">
											<option value="${gradeList.gradeId}" ${gradeList.gradeId eq courseList.gradeId? "selected":""}>${gradeList.gradename}</option>
										 </c:forEach>
									</select>
								</div>
							</div>
					...
					<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="text" name="courseprice" id="courseprice" lay-verify="title"
									autocomplete="off" placeholder="请输入课件价格" class="layui-input" 
									onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
									 value="${courseList.courseprice}">
							</div>
					</div>
					<div class="layui-form-item">
						...
						<div class="layui-input-block"><!-- layui-input-inline -->
							<select name="papeId" lay-verify="required" lay-search="">
								...
								<c:forEach items="${paperList}" var="paperList">
									<option value="${paperList.papeId}"  ${paperList.papeId eq courseList.papeId? "selected":""}>${paperList.papername}</option>
								</c:forEach>
							</select>
						</div>
					</div>
				...
				<div class="layui-row" style="margin-top:25px;">
					<div class="layui-col-md1 layui-col-md-offset4">
						 <button type="button" class="layui-btn" lay-filter="editSave"lay-submit="" >确认保存</button>
					</div>
					<div class="layui-col-md1"style="margin-left: 40px;">
						<button type="button" class="layui-btn layui-btn-warm" id="return" onClick="javascript :history.back(-1);">关闭</button>
					</div>
				</div>
			</div>
		</div>
	   ...
	   <script>
    	 ...
    	 layui.use([ 'form', 'upload' ], function(){
		    ...
		    form.on('submit(editSave)', function(data) {
             ...
             //显示图片名称
		     var videoPath = "${courseList.video}";
		     if(videoPath!=""){
		           var index= videoPath.lastIndexOf("/");
		           var videoName=videoPath.substring(index+1,videoPath.length);
		           if (videoName != "") $("#showApellation").text(videoName);
		           $("#showApellation").val(videoName);
		     }
	     ...
	     file.on('change', function(e){
		     ...
		     if (imgfFile != undefined) {
			    if (!regexImageFilter.test(e.currentTarget.files[0].type)) {
	    		   ...
	    		   $("#showApellation").val(e.currentTarget.files[0].name);
			    }
		     }
	     });
    	 //修改课件
    	 function editSave(data){
    	      var video= $("#showApellation").val();
                 ...
                 $("#formUpdate").ajaxSubmit({
					...
					success : function(data, textStatus) {
						...
						if (data.state == 'ok') {
							layer.msg(data.msg);
							$("#return").click();
							$.post("${ctx}/user/insertLog.do",{operType:"修改课件包信息"},function(){});
						} else {
							layer.msg(data.msg);
						}
					},
