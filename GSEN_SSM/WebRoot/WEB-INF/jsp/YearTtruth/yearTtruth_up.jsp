...
<html>
  <head>
    ...
    <title>My JSP 'yearTtruth_up.jsp' starting page</title>
      ...
      <form class="layui-form" id="formInsert" action="${ctx}/yearTtruth/IntPatchPicture.do" method="post"  enctype="multipart/form-data">
			...
			<div class="layui-form" style="padding: 2px 80px;margin-top: -10px;" >
						...
						<div class="layui-inline ">
							...
							<div class="layui-input-block" style="width:340px;">
								<input type="text" name="papername" id="papername" lay-verify="title"
									autocomplete="off" placeholder="请输入试卷名称" class="layui-input" value="${paperList.papername}">
							</div>
						</div>
							...
							<div class="layui-input-block" style="width:340px;">
								<select name="papertypeId" id="papertypeId">
									...
									<c:forEach items="${papertypeList}" var="papertypeList">
										<option value="${papertypeList.papertypeId}" ${papertypeList.papertypeId eq paperList.papertypeId? "selected":""}>${papertypeList.papertype}</option>
									</c:forEach>
								</select>
							</div>
						</div>
							...
							<div class="layui-input-block" style="width:340px;">
								<select name="gradeId" id="gradeId">
									...
									<c:forEach items="${gradeList}" var="gradeList">
										<option value="${gradeList.gradeId}" ${gradeList.gradeId eq paperList.gradeId? "selected":""}>${gradeList.gradename}</option>
									</c:forEach>
								</select>
							</div>
						</div>
							...
							<div class="layui-input-block" style="width:340px;">
								<select name="subjectId" id="subjectId">
									...
									<c:forEach items="${subjectList}" var="subjectList">
										<option value="${subjectList.subjectId}" ${subjectList.subjectId eq paperList.subjectId? "selected":""}>${subjectList.subject}</option>
									</c:forEach>
								</select>
							</div>
						</div>
					...
					<div class="layui-form-item">
						 <div class="layui-inline " >
								...
								<div class="layui-input-block"style="width:340px;" >
									...
									autocomplete="off" placeholder="请输入试卷总分" class="layui-input" value="${paperList.paperscore}" onkeyup="this.value= this.value.match(/\d+(\.\d{0,1})?/) ? this.value.match(/\d+(\.\d{0,1})?/)[0] : ''">
								</div>
						</div>
							<div class="layui-inline"style="float: right;margin-right:0px;">
								...
								<div class="layui-input-block" style="width:340px;">
									...
									autocomplete="off" placeholder="请输入考试限时" class="layui-input" value="${paperList.testtime}" >
								</div>
							</div>
					</div>
					<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="text" name="notes" id="notes" lay-verify="title"
									autocomplete="off" placeholder="请输入注意事项" class="layui-input" value="${paperList.notes}" >
							</div>
					</div>
				</div>
				...
				<div  class="layui-row">
				   ...
				   <div class="layui-col-xs6">
				         ...
				         <button type="button" class="layui-btn" onclick="editSave(1)" style="float: right;">修改后发布</button>
				         <button type="button" class="layui-btn" onclick="editSave(0)" style="float: right;">修改为草稿</button>
	   ...
	   <script>
	         ...
	         layui.use('form',function(){
	            ...
	            IntPicture=[];
	             var Id = "${paperList.papeId}";
	            if(Id!="" && Id!=null){
	                 $.post("${ctx}/yearTtruth/showTitleType.do",{id:Id},function(data){
	                     var data=data.data;
	                       for ( var i = 0; i < data.length; i++) {
							   	var id = "div_headline_" + (i + 1);
							   $("#div_count").append("<div id='" + id + "'><div class='div-margin form-group fff-wrap'>" + 
										   "<span class='title-span'>" + (i + 1) + "、</span>" + 
										   "<input class='input-text input-qbc input-radius auto title-left input-span' id='input_dart_" + (i + 1) + "' type='text' title='标题' value='" + data[i].titletype + "'>" + 
										   ...
										   "</ul></div><span class='layui-icon layui-icon-delete' title='删除' onclick='deleteDiv(event)'></span>" + 
										   "</div></div></div>");
	         	               arrTitle.push(id);
	         	               var titletypeId = data[i].titletypeId;
	         	               if(titletypeId>0){
	         	                    $.ajaxSettings.async = false;//同步  ，保证代码顺序执行
	         	                    $.post("${ctx}/yearTtruth/showTitle.do",{id:titletypeId},function(datas){
	         	                         var datas=datas.data;
	         	                         for(var o=0;o<datas.length;o++){
	         	                             var type=datas[o].title_type;
	         	                             var fatherLength = arrTopic.length + 1;
								             var Id = "father_dart_" + fatherLength;
								             var titleA=datas[o].titleA;
								             while(titleA.indexOf("reveal_photo.do")!=-1){
								                  titleA=titleA.replace("reveal_photo.do", "reveal_photo_up.do");
								             }
								             var titleA1=datas[o].titleA;
								             var dataSplit=titleA1.split("/yearTtruth/reveal_photo.do?file=");
								             for ( var int = 0; int < dataSplit.length; int++) {
												 var index=dataSplit[int].indexOf('.');
												 if(index>0){
												     var length= dataSplit[int].indexOf('">');
												     var replace=dataSplit[int].substring(0,length);
												     var fg=replace.split(".");
												     for ( var j = 0; j < fg.length; j++) {
														  IntPicture.push(fg[j]);
												 	 }
												 }
											 }
								             ...
								             if(type==118 || type==119){//单选题或多选题
													   ...
													   "<span class='topic-span'>" + (o + 1) + "、</span>" + 
													   "<div class='title_content auto title-left input-span input-height' id='dart_" + fatherLength + "' contenteditable='true' onclick='getCursortPosition(event);'>" + titleA + "</div>" + 
													   "<div id='option_" + fatherLength + "'>" + 
													   ...
													   "<input id='answer_" + fatherLength + "' class='input-text input-radius auto title-left input-span answer_content score-input' type='text' title='答案'>" + 
													   "</div><div class='title_option'><span class='score-span'>本题得分：</span>" + 
													   "<input id='score_" + fatherLength + "' class='input-text input-radius auto title-left input-span score_content score-input' type='text' title='本题得分'>" + 
													   "</div></div>");
													   //选项
													   ...
													   for (var p = 1; p < option.length; p++) {
														   ...
														   $("#option_" + fatherLength).append("<div class='title_option'><i class='icon_wj'>" + loading[(p - 1)] + "、</i>" +
														   "<div class='title_content auto title-left' id='dart_list_" + fatherLength + "_" + p + "' contenteditable='true' onclick='getCursortPosition(event)'>"+optionSubstr+"</div></div>");
	     											        var dataSplit=optionSubstr.split("/yearTtruth/reveal_photo_up.do?file=");
								             		        for ( var int = 0; int < dataSplit.length; int++) {
												 		        var index=dataSplit[int].indexOf('.');
												 		        if(index>0){
												     		        var length= dataSplit[int].indexOf('">');
	     											    ...
	     											    $("#answer_" + fatherLength).val(datas[o].answerA);//答案
								                       $("#score_" + fatherLength).val(datas[o].titlescore);//本题得分
								             }else if(type==120){//问答题
								                  $("#" + id).append("<div class='div-margin form-group fff-wrap' id='" + Id + "'>" + 
													   "<span class='topic-span'>" + (o + 1) + "、</span>" + 
													   "<div class='title_content auto title-left input-span input-height' id='dart_" + fatherLength + "' contenteditable='true' onclick='getCursortPosition(event)'>" + titleA + "</div>" + 
													   ...
													   "<span class='layui-icon layui-icon-picture-fine' title='添加图片' onclick='appPicture()'></span>" +
													   "<span class='layui-icon layui-icon-up' title='上移' onclick='MoveUpDiv(event)'></span>" + 
													   "<span class='layui-icon layui-icon-down' title='下退' onclick='MoveDownDiv(event)'></span>" + 
													   "<span class='layui-icon layui-icon-delete' title='删除' onclick='deleteDiv(event)'></span>" + 
													   "</div><div class='title_option'><i class='icon_wj'>答案：</i>" + 
													   "<div class='answer_content auto title-left' id='answer_" + fatherLength + "' contenteditable='true' onclick='getCursortPosition(event)'></div>" + 
													   "</div><div class='title_option'><span class='score-span'>本题得分：</span>" + 
													   "<input id='score_" + fatherLength + "' class='input-text input-radius auto title-left input-span score_content score-input' type='text' title='本题得分'>" + 
													   "</div></div>");
									                  ...
									                  $("#answer_" + fatherLength).html(answerA);//答案
								                      $("#score_" + fatherLength).val(datas[o].titlescore);//本题得分
								                      var dataSplit=answerA.split("/yearTtruth/reveal_photo_up.do?file=");
								             		        for ( var int = 0; int < dataSplit.length; int++) {
												 		        var index=dataSplit[int].indexOf('.');
												 		        if(index>0){
												     		        var length= dataSplit[int].indexOf('">');
							                      ...
							                      "<div id='option_" + fatherLength + "'><div class='title_option'><i class='icon_wj'>A、</i>" + 
							   	                       "<div class='title_content auto title-left' id='dart_list_" + (o + 1) + "_1' contenteditable='false'>正确</div></div>" + 
							      	                    "<div class='title_option'><i class='icon_wj'>B、</i>" + 
				   				                       "<div class='title_content auto title-left' id='dart_list_" + (o + 1) + "_2' contenteditable='false'>错误</div></div>" + 
			   					                       "</div>"+
			   				                       "<div class='title_option'><i class='icon_wj'>答案：</i>" + 
							                       "<div class='answer_content auto title-left' id='answer_" + fatherLength + "' contenteditable='true'></div>" + 
							                       "</div>"+
							   
							                       "<div class='title_option'><span class='score-span'>本题得分：</span>" + 
							                       "<input id='score_" + fatherLength + "' class='input-text input-radius auto title-left input-span score_content score-input' type='text' title='本题得分' onkeyup='clearNoNum(this)' onblur='clearNoNum(this)'>" + 
							                       "</div></div>");
							                        $("#answer_" + fatherLength).html(datas[o].answerA);//答案
								                    $("#score_" + fatherLength).val(datas[o].titlescore);//本题得分
								             }
								              	arrTopic.push([Id, i, type, datas[o].serial]);
	       ...
	       function MoveDownDiv(e) {
	        ...
	        function IntLiTopic(e, index){
		     ...
		     //arrTopic  [题目id， 标题id，题目类型，序号]
		     //           div的id，当前题目的id是属于哪种题目类型(是界面上的id，从0开始)， ，
	        }
	        //修改保存试卷
	        function editSave(saveType){
				      ...
				      for (var o = 0; o < arrTopic.length; o++) {
					      ...
					      if (arrTopic[o][2] == 118 || arrTopic[o][2] == 119) {//单选题、多选题
						      ...
						      for (var p = 0; p < eLength; p++) {
						          var option=$("#" + e.children[2].children[p].children[1].id).html();
						          while(option.indexOf("reveal_photo_up.do")!=-1){
								        option=option.replace("reveal_photo_up.do", "reveal_photo.do");
								  }
						      ...
						      while(answer.indexOf("reveal_photo_up.do")!=-1){
								    answer=answer.replace("reveal_photo_up.do", "reveal_photo.do");
							  }
					      } else if (arrTopic[o][2] == 121) {//判断题
					           var answerIndex = e.children[3].id.split("_");
						      answer = $("#" + e.children[4].children[1].id).html();
					      }
					      var title=e.children[1].innerHTML;
					      while(title.indexOf("reveal_photo_up.do")!=-1){
								title=title.replace("reveal_photo_up.do", "reveal_photo.do");
						  }
					      //IntarrTopic  [题目id(题目、题目附图)， 题目类型id(当前题目是属于哪种题目类型(界面上的id),新增保存返回id)，题目类型(118,119,120,121)，序号， 题目得分、选项，答案]
					      IntarrTopic.push(title, arrTopic[o][1]+"titleType", arrTopic[o][2], arrTopic[o][3], $("#" + e.children[e.children.length - 1].children[1].id).val(), optionType, answer);
				      }
				      var paperId= "${paperList.papeId}";
				       $.post("${ctx}/yearTtruth/editYearTruth.do",{
				          paperId:paperId,
	                      papername: papername,
	                      ...
	                      saveType:saveType
	                       ...
	                       if(data.state == 'ok'){
	                            ...
	                            $.post("${ctx}/user/insertLog.do",{operType:"修改历年真题信息"},function(){});
	                       }else{
