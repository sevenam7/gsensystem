		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]"' + multiple + '>';// accept=".xls,.xlsx,.doc,.docx,.pdf" 
	            	html += '					</div>';
	            	html += '				</div>';
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div><div id="preview" class="upload_preview"></div></div>';
					html += '		</div>';
...
//					html += '		<div id="uploadInf" class="upload_inf"></div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var fileImgSrc = "/assets/upload/control/images/fileType/";
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="" href="javascript:;" class="imgBox">';
					html += '		<div class="uploadImg" style="">';				
					html += '			 <span  id="uploadImage_' + file.index + '" >' + file.name + '</span>';
					html += '		</div>';
					html += '	</a>';
					html += '	<p id="uploadProgress_'+file.index+'" class="file_progress"></p>';
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">上传失败，请重试</p>';
